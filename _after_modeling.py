...
# target 설정
reg_dataset['target'] = reg_dataset['가계지출_소비지출_의류신발구입비_비율']

drop_cols = (
    item_columns +  # 12개 소비 항목 원금
    [col + '_log' for col in item_columns] +  # 로그 스케일링된 금액
    [col + '_비율' for col in item_columns] +  # 비율 컬럼 전부 (의류신발 포함)
    ['가계지출_소비지출금액', '가계지출금액'] +  # 총 소비금액 관련
    ['target']  # 타겟 변수
)
...
pipeline = Pipeline([
    # 1) 스케일링
    ('scaler', StandardScaler()),
    # 2) 차수 2 다항항 생성 (interaction_only=True로 곱항만 만들거나,
    #    include_bias=False로 상수항 제거)
    ('poly', PolynomialFeatures(degree=2, interaction_only=True, include_bias=False)),
    # 3) LassoCV 로 규제를 함께 적용
    ('lasso', LassoCV(cv=5, random_state=42, max_iter=10000))
])

X = reg_dataset.drop(columns=drop_cols, errors='ignore')
y = reg_dataset['target']
...
print("Poly RMSE:", np.sqrt(mean_squared_error(y_test, y_pred)))
print("Poly R²:  ", r2_score(y_test, y_pred))

# Lasso 모델과 변환된 피처 이름 가져오기
...
print(coef_df.head(10))  # 절대값 큰 상위 10개 항목

from sklearn.model_selection import cross_val_score
cv_r2 = cross_val_score(pipeline, X, y, cv=5, scoring='r2')
print("5-fold CV R²:", cv_r2, "→ mean:", cv_r2.mean())
